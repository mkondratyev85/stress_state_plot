...
class StressStateValues:
    def __init__(
        ...
        if sigma1 is not None and sigma2 is not None and sigma3 is not None:
            ...
            p is not None and tau is not None and (mu_s is not None or phi is not None)
        ):
            self.parse_not_from_sigmas(p, tau, mu_s, phi)
        else:
            raise ValueError(
                "You must set values either for (sigma1, sigma2, sigma3) or for (p, tau mus_s or phi)"
            )
    ...
    def __init__(
        ...
        def parse_single_sigma(maybe_sigma):
            ...
            if len(maybe_sigma) == 2:
                direction, dip = maybe_sigma
                return Plane(direction, dip)
            raise ValueError(
                "Value for sigma orientation must be either of type Plane or tuple of two float values"
            )
        ...
        def get_orientation_from_direction_and_another_sigma(another_sigma, direction):
            """An ugly way of getting orientation of an axis given by direction and orientation of perpendicular axis."""
        ...
        if sigma1_sigma3:
            ...
            sigma3 = get_orientation_from_direction_and_another_sigma(
                another_sigma=sigma1, direction=direction
            )
        elif sigma3_sigma1:
            sigma3_dir, sigma3_dip, direction = sigma3_sigma1
            sigma3 = Plane(sigma3_dir, sigma3_dip)
            sigma1 = get_orientation_from_direction_and_another_sigma(
                another_sigma=sigma3, direction=direction
            )
        else:
        ...
        if none_sigmas > 1:
            raise ValueError(
                "Set orientations for at least two sigmas or one sigma and direction of another sigma"
            )
