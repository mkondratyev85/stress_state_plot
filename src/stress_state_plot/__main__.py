...
def parse_sigma_orientation(string_value):
    ...
    if len(values) != 2:
        raise ValueError(
            "Sigma orientation must be set as two floats separated by a space\n"
...
def parse_sigma_and_direction(sigma_and_direction):
    ...
    try:
        ...
        raise ValueError(
            f"Error while parsing sigma and direction argument {sigma_and_direction}"
        )
...
@click.option("--sigma1_value", help="Value of sigma1.", type=float)
@click.option("--sigma2_value", help="Value of sigma2.", type=float)
@click.option("--sigma3_value", help="Value of sigma3.", type=float)
@click.option("--sigma1_orientation", help="Orientation of sigma1.", type=float)
@click.option("--sigma2_orientation", help="Orientation of sigma2.", type=float)
@click.option("--sigma3_orientation", help="Orientation of sigma3.", type=float)
@click.option(
    "--sigma1_orientation_and_sigma3_direction",
    help="Set orientations only via single sigma1_orientationa and direction of sigma3",
)
@click.option(
    "--sigma3_orientation_and_sigma1_direction",
    help="Set orientations only via single sigma3_orientationa and direction of sigma1",
)
@click.option("--tau_f", help="Cohesion factor", type=float)
@click.option("--k_f", help="Coefficient of friction", type=float)
    ...
    try:
        stress_state = StressState(
            orientations={
                ...
                "sigma1_sigma3": parse_sigma_and_direction(
                    sigma1_orientation_and_sigma3_direction
                ),
                "sigma3_sigma1": parse_sigma_and_direction(
                    sigma3_orientation_and_sigma1_direction
                ),
            },
            values={
